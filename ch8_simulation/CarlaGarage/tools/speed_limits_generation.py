...
def compute_speed_limit_map(carla_port, map_name, output_dir):
  ...
  # set variables up to interpolate the rest of the locations, for which we couldn't compute the speed limits earlier
  ...
  np.save(f'{output_dir}/{map_name}_speed_limits.npy', d)

if __name__ == '__main__':
  parser = argparse.ArgumentParser()
  parser.add_argument("--carla_port", type=int, default=2000)
  parser.add_argument("--map_name", type=str, default='Town07')
  parser.add_argument("--output_dir", type=str, default='./carla_garage/team_code/speed_limits')
  args = parser.parse_args()
  print(f'Compute speed limit for {args.map_name}')
  compute_speed_limit_map(args.carla_port, args.map_name, args.output_dir)
