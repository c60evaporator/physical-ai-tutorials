"""
Child of the autopilot (PDM-Lite) that collects data with a nuScenes-style camera rig.

Based on data_agent.py, but replaces the single front camera (+ augmented/semantics/depth
variants) with the 6 RGB cameras of the nuScenes vehicle (CAM_FRONT, CAM_FRONT_LEFT,
CAM_FRONT_RIGHT, CAM_BACK, CAM_BACK_LEFT, CAM_BACK_RIGHT). Images are stored in
nuScenes-named folders and the camera calibration (intrinsics + mounting extrinsics)
is written once per route to sensor_calibration.json.
"""
...
# nuScenes camera rig (v1.0 calibrated_sensor) converted to CARLA coordinates.
...
NUSCENES_CAMERAS = [
    {'id': 'CAM_FRONT', 'x': 0.28, 'y': 0.0, 'z': 1.51, 'roll': 0.0, 'pitch': 0.0, 'yaw': 0.0, 'fov': 70},
    {'id': 'CAM_FRONT_LEFT', 'x': 0.15, 'y': -0.50, 'z': 1.52, 'roll': 0.0, 'pitch': 0.0, 'yaw': -55.0, 'fov': 70},
    {'id': 'CAM_FRONT_RIGHT', 'x': 0.16, 'y': 0.50, 'z': 1.52, 'roll': 0.0, 'pitch': 0.0, 'yaw': 55.0, 'fov': 70},
    {'id': 'CAM_BACK', 'x': -1.37, 'y': 0.0, 'z': 1.57, 'roll': 0.0, 'pitch': 0.0, 'yaw': 180.0, 'fov': 110},
    {'id': 'CAM_BACK_LEFT', 'x': -0.38, 'y': -0.48, 'z': 1.56, 'roll': 0.0, 'pitch': 0.0, 'yaw': -110.0, 'fov': 70},
    {'id': 'CAM_BACK_RIGHT', 'x': -0.36, 'y': 0.47, 'z': 1.61, 'roll': 0.0, 'pitch': 0.0, 'yaw': 110.0, 'fov': 70},
]

# nuScenes image resolution
NUSCENES_CAMERA_WIDTH = 1600
NUSCENES_CAMERA_HEIGHT = 900
...
class DataAgentNuScenes(AutoPilot):
  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    # *_QUALIFIER tracks; the 6 camera rig exceeds the 4 rgb camera cap of Track.MAP.
  ...
  def save_calibration(self):
    ...
    for camera in NUSCENES_CAMERAS:
      intrinsic = t_u.calculate_intrinsic_matrix(fov=camera['fov'],
                                                 height=NUSCENES_CAMERA_HEIGHT,
                                                 width=NUSCENES_CAMERA_WIDTH)
    ...
    calibration['lidar'] = {
        'position': self.config.lidar_pos,
        'rotation': self.config.lidar_rot,
    }
  ...
  def tick(self, input_data):
    ...
    if self.save_path is not None and (self.datagen or self.tmp_visu):
      camera_images = {}
      for camera in NUSCENES_CAMERAS:
        camera_images[camera['id']] = input_data[camera['id']][1][:, :, :3]
    else:
      camera_images = {camera['id']: None for camera in NUSCENES_CAMERAS}
    ...
    if self.tmp_visu:
      self.visualuize(bev_semantics['rendered'], camera_images['CAM_FRONT'])

    result.update(camera_images)
    result.update({
        'lidar': lidar_360,
